...
module BremountaPlate()
{
    ...
    difference()
    {
        union()
        {
            ...
            translate([ 0, PLATE_DEPTH / 5, 0 ])
                cube([ SOCKET_WIDTH * 1.75, PLATE_DEPTH / 5 * 3, WOOD_BOARD_THICKNESS ]);
        }
        for (i = [0:1])
        {
            translate([ SOCKET_WIDTH / 2, (PLATE_DEPTH - BREMOUNTA_SCREW_DIST) / 2 + BREMOUNTA_SCREW_DIST * i, 0 ])
            {
            ...
            {
                translate([
                    SOCKET_WIDTH * 1.75, (PLATE_DEPTH - WOOD_BOARD_SCREW_DIST) / 2 + WOOD_BOARD_SCREW_DIST * i,
