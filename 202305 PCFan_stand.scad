...
difference() {
    ...
    translate([FAN_DIAMETER / 2, FAN_DEPTH + 0.01, FAN_DIAMETER / 2 + MATERIAL_THICKNESS + HEIGHT_FROM_FLOOR])
        rotate([90, 0, 0])
            cylinder(h = FAN_DEPTH + 0.02, d = FAN_DIAMETER, $fn = FN);
    ...
    for (i = [0:1]) {
        ...
        translate([0, FRONT_OVERHANG + MATERIAL_THICKNESS + M3_HOLE, - 0.01]) {
            translate([SCREW_OFFSET + FAN_SCREW_DISTANCE * i, 0, 0])
                cylinder(h = MATERIAL_THICKNESS + 0.02, d = M3_HOLE, $fn = FN);

            if (FAN_DIAMETER == 80 || FAN_DIAMETER == 120) {
                translate([FAN_DIAMETER / 3 * (i + 1), UNC14_EDGE_OFFSET, 0])
                    cylinder(d = UNC14_HOLE, h = MATERIAL_THICKNESS + 0.02, $fn = FN);
            }
            if (FAN_DIAMETER == 140) {
