...
difference() {
    rotate([90, 0, 90])
        linear_extrude(height = FAN_DIAMETER) {
            polygon([
                    [0, 0],
                    [FAN_DEPTH, 0],
                    [FAN_DEPTH, MATERIAL_THICKNESS],
                    [MATERIAL_THICKNESS + FAN_DEPTH / 5, MATERIAL_THICKNESS],
                    [MATERIAL_THICKNESS + FAN_DEPTH / 5, MATERIAL_THICKNESS + SCREW_OFFSET * 2 + HEIGHT_FROM_FLOOR],
                    [FAN_DEPTH / 5, MATERIAL_THICKNESS + SCREW_OFFSET * 2 + HEIGHT_FROM_FLOOR],
                    [FAN_DEPTH / 5, MATERIAL_THICKNESS],
    ...
    for (i = [0:1]) {
        translate([SCREW_OFFSET + FAN_SCREW_DISTANCE * i, FAN_DEPTH / 10 - 0.01,
                    MATERIAL_THICKNESS + HEIGHT_FROM_FLOOR + SCREW_OFFSET])
            rotate([90, 0, 0]) screw(SCREW_DIAMETER, FAN_DEPTH + 0.02, true);

        translate([SCREW_OFFSET + FAN_SCREW_DISTANCE * i, FAN_DEPTH / 2, - 0.01])
            cylinder(h = MATERIAL_THICKNESS + 0.02, d = M3_HOLE, $fn = 100);
    }

    HEX_OD = FAN_DEPTH - (FAN_DEPTH / 5) - MATERIAL_THICKNESS - 2;
    usable_width = FAN_DIAMETER - SCREW_OFFSET * 3;
    translate([(FAN_DIAMETER - hexagon_get_used_width(usable_width, HEX_OD)) / 2,
                    FAN_DEPTH / 5 + MATERIAL_THICKNESS + (HEX_OD - hexagon_get_used_width(HEX_OD + 2, HEX_OD)) / 2, 0])
        hexagon_pattern(usable_width, HEX_OD + 2, MATERIAL_THICKNESS, HEX_OD);
}
