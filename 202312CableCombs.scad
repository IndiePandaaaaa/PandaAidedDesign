...
function comb_width(cable_count, cable_rows, cable_od, cable_distance, outer_border = 1) = outer_border * 2 + (
  cable_count / cable_rows) * (cable_od + cable_distance) - cable_distance;

function comb_depth(cable_rows, cable_od, cable_distance, outer_border = 1) = outer_border * 2 + cable_rows * (cable_od
  + cable_distance) - cable_distance;

function comb_chamfer(cable_od) = cable_od / 2;

function comb_mounting_distance(cable_count, cable_rows, cable_od, screw_od = core_hole_M3(), cable_distance = 1,
outer_border = 1) = comb_width(cable_count, cable_rows, cable_od, cable_distance, outer_border) + screw_od * 2 + 1;
...
module comb(cable_count, cable_rows, cable_od, cable_distance = 1, thickness = 2, with_chamfer = true, tolerance = .1) {
  ...
  difference() {
    ...
    translate([outer_border + cable_od / 2, outer_border + cable_od / 2, -thickness * 0.25])
      cube([width - outer_border * 2 - cable_od * cutout, depth - outer_border * 2 - cable_od * cutout, thickness * 1.5
        * cutout]);
  }
}

module threaded_comb(cable_count, cable_rows, cable_od, cable_distance = 1, screw_od = 3, offset_mounting = false,
tolerance = .1) {
    ...
    translate([thickness, 0, 0]) {
      if (offset_mounting) {
        ...
        translate([0, -comb_depth(cable_rows, cable_od, cable_distance), 0]) {
          ...
          translate([comb_width(cable_count, cable_rows, cable_od, cable_distance), comb_depth(cable_rows, cable_od,
          cable_distance) - comb_chamfer(cable_od), 0])
            rotate([0, 0, 45]) cube([comb_chamfer(cable_od) * 2, comb_chamfer(cable_od) * 2, thickness]);
        }
      }
...
module angled_comb(cable_angle, cable_radius, cable_count, cable_rows, cable_od, cable_distance, thickness = 2,
outer_border = 1, tolerance = .1) {
  ...
  module segments(cable_angle, cable_radius, width, cable_count, cable_rows, cable_od, cable_distance, thickness, cutout
  = false, tolerance = .1, angle_steps = 45) {
    ...
    for (i = [0:cable_angle / angle_steps]) {
      rotate([angle_steps * i, 0, 0])
        translate([0, cable_radius - width / 2, -thickness_cutout / 2]) {
          ...
          translate([comb_chamfer(cable_od) - tolerance * blockw, -comb_chamfer(cable_od) / 2 - blockw * tolerance / 2,
            0])
  ...
  translate([0, 0, thickness / 2]) union() {
    difference() {
      rotate([0, 90, 0]) translate([0, 0, comb_chamfer(cable_od)]) border_inside(cable_angle, cable_radius, cyld,
      cable_od, cylw, thickness, tolerance);
      segments(cable_angle, cable_radius, cylw, cable_count, cable_rows, cable_od, cable_distance, thickness, true, .15,
      45);
    }
    segments(cable_angle, cable_radius, cylw, cable_count, cable_rows, cable_od, cable_distance, thickness, false, .1,
    45);
  }
}
