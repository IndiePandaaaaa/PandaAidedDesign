...
DISTANCES = [[140, 125], [120, 105], [80, 71.5]];

module pc_fan_stand(fan_diameter, fan_screw_distance, fan_depth = 25, front_overhang = 0, stand_height_offset = 5, thickness = 3) {
  screw_diameter = 5.2;
  stand_depth = fan_depth * 1.5;

  translate(v=[-fan_diameter / 2, 0, 0]) {
    cube(size=[fan_diameter, stand_depth, thickness], center=false);
    cube(size=[fan_diameter, thickness, stand_depth], center=false);
  }
}

pc_fan_stand(fan_diameter=120, fan_screw_distance=105);
...
module fan_stand(FAN_DIAMETER, FAN_SCREW_DISTANCE, FAN_DEPTH = 25) {
  ...
  difference() {
    ...
    for (i = [0:1]) {
      translate(
        [
          SCREW_OFFSET + FAN_SCREW_DISTANCE * i,
          FRONT_OVERHANG / 2 - 0.1 - MATERIAL_THICKNESS / 2,
      ...
      border_offset = 5;
      translate([border_offset + i * (FAN_DIAMETER - border_offset * 2), 12, MATERIAL_THICKNESS + .1]) screw(
          3.5, 12,
          true
        );
    }
    ...
    translate(
      [
        (FAN_DIAMETER - hexagon_get_used_width(usable_width, HEX_OD)) / 2,
        FRONT_OVERHANG + MATERIAL_THICKNESS + (HEX_OD - hexagon_get_used_width(usable_depth, HEX_OD)) / 2,
        0,
      ]
    )
      rotate([180, 0, 0])
